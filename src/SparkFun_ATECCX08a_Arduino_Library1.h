...
class ATECCX08A {
  public:
    ...
    //By default use Wire, standard I2C speed, and the default ADS1015 address
		// checking which board we are using and selecting a Serial debug that will work.
	#if defined(ARDUINO_ARCH_APOLLO3) || defined(ESP32) || defined (ARDUINO)
  ...
  boolean lockDataSlot(uint16_t slot);
	...
	bool    getSlotLockStatus(uint16_t slot);
